...
class AccuWeather_forecast:
    def __init__(self):
        self.zipcode = "15282" #using Duquesne zip code as the default for the data we're collecting.
        self.hourly_data = []
        self.daily_data = []
    ...
    def hourly_forecast(self):
        ...
        for i in range(num_hour_day1):
            temp_dic_day1 = {}
            temp_dic_day1["Hour"] = soup_day1.find_all('h2', class_="date")[i].get_text().replace("\n", "").split("\t")[0]
            temp_dic_day1["Date"] = soup_day1.find_all('h2', class_="date")[i].get_text().replace("\n", "").split("\t")[3]+str(self.now.year)
            lst_temp = temp_dic_day1["Date"].split("/")
            temp_dic_day1["Date"] = str(num_to_month[int(lst_temp[0])])+lst_temp[1]
            ...
            #temp_dic_day1["Real Feel (in F)"] = soup_day1.find_all('span', class_="real-feel")[i].get_text()[15:17]
        ...
        for i in range(num_hour_day2):
            temp_dic_day2 = {}
            temp_dic_day2["Hour"] = soup_day2.find_all('h2', class_="date")[i].get_text().replace("\n", "").split("\t")[0]
            temp_dic_day2["Date"] = soup_day2.find_all('h2', class_="date")[i].get_text().replace("\n", "").split("\t")[3]+str(self.now.year)
            lst_temp = temp_dic_day2["Date"].split("/")
            temp_dic_day2["Date"] = str(num_to_month[int(lst_temp[0])])+lst_temp[1]
            ...
            #temp_dic_day2["Real Feel (in F)"] = soup_day2.find_all('span', class_="real-feel")[i].get_text()[15:17]
            temp_dic_day2["Description"] = soup_day2.find_all('span', class_="phrase")[i].get_text().replace("\n", "").replace("\t","")
            date_time = self.now.strftime("%b%d%Y %I%p")
            #date_time = self.now.strftime("%b%d%Y %H:%M:%S")
    ...
    def daily_forecast(self):
        ...
        for i in range(10):
            temp_dic = {}
            temp_dic["Date"] = soup.find_all('span', class_="module-header sub date")[i].get_text()+str(self.now.year)
            lst_temp = temp_dic["Date"].split("/")
            temp_dic["Date"] = str(num_to_month[int(lst_temp[0])])+lst_temp[1]
            ...
            #temp_dic["Day"] = soup.find_all('span', class_="module-header dow date")[i].get_text()
...
#     zipcode = "15282" ## change this line to your desired zipcode if necessary
